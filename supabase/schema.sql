...
-- em "Run". Pode rodar mais de uma vez sem problema (usa IF NOT EXISTS).
-- =============================================================================
...
-- Cada linha é UMA rodada pontuada de UMA equipe.
...
comment on table score_rounds is 'Pontuação de cada rodada (1 ou 2) de cada equipe, calculada a partir do fichário.';
...
comment on table rubric_scores is 'Avaliação da Rúbrica Equipe Destaque (máx. 100 pontos) por equipe.';
...
-- -----------------------------------------------------------------------------
-- VIEW: melhor rodada de cada equipe (a que vale para a classificação)
...
select distinct on (team_id)
  ...
  round_time_seconds
from score_rounds
order by team_id, final_score desc, round_time_seconds asc nulls last, created_at asc;

-- -----------------------------------------------------------------------------
-- VIEW: placar geral (ranking) por equipe, juntando melhor rodada + rubrica
-- -----------------------------------------------------------------------------
...
from teams t
left join team_best_round b on b.team_id = t.id
left join rubric_scores r on r.team_id = t.id
order by t.grade, best_round_score desc, best_round_time_seconds asc nulls last;

-- =============================================================================
-- SEGURANÇA (RLS)
-- Nesta primeira versão, liberamos leitura e escrita para simplificar o uso
-- com a chave "anon" a partir do app (o acesso ao app já é protegido por uma
-- senha simples de professor(a) no próprio Next.js). Se depois você quiser
-- reforçar a segurança no nível do banco, troque estas políticas por regras
-- que exijam um usuário autenticado do Supabase Auth.
-- =============================================================================
